...
// Keep the protocol sensors in a stable visual order while allowing custom sensors after them.
const sensorOrder = [
  'left_travel', 'right_travel', 'shovel_lift', 'rear_support',
  'cutter_extend', 'cutter_horizontal', 'cutter_vertical',
  'conveyor_1_forward', 'conveyor_1_reverse', 'starwheel_forward',
  'starwheel_reverse', 'warning_bell', 'conveyor_2_start', 'oil_pump_start',
  'travel_enable', 'emergency_stop', 'cutter_high_speed', 'cutter_low_speed',
  'wind_speed', 'temperature', 'humidity', 'methane',
  'tilt_x', 'tilt_y', 'tilt_z', 'hydraulic_tank_level',
  'thrust_cylinder_level', 'shield_cylinder_level', 'left_thrust_pressure',
  'right_thrust_pressure', 'rotation_pressure', 'total_thrust_pressure',
  'support_pressure'
]
const sensorOrderIndex = (sensor: any) => {
  const key = `${sensor.type || ''} ${sensor.name || ''} ${sensor.id || ''}`.toLowerCase()
  const index = sensorOrder.findIndex(name => key.includes(name))
  return index < 0 ? sensorOrder.length : index
}
const orderedSensors = computed(() => [...((device.value as any)?.sensors || [])].sort((a, b) => {
  const orderDiff = sensorOrderIndex(a) - sensorOrderIndex(b)
            ...
            <el-tab-pane label="概览" name="overview">
              ...
              <div class="overview-content">
                ...
                <el-row :gutter="15" class="sensor-cards" v-if="device.sensors.length > 0">
                  <el-col :span="8" v-for="sensor in orderedSensors" :key="sensor.id">
...
<style scoped>
.device-detail { max-width: 1440px; margin: 0 auto; }.page-header { margin-bottom: 24px; }.header-left h2 { font-size: 25px; letter-spacing: -.02em; }.header-left h2::after { content: 'DEVICE DETAIL / LIVE CONTROL'; display: block; margin-top: 5px; color: var(--color-cyan); font: 10px/1 'Roboto Mono', monospace; letter-spacing: .12em; }
.summary-bar { border-radius: 8px; background: var(--bg-secondary); border-color: var(--border-color); }.summary-item { min-width: 116px; }.summary-label { color: var(--text-muted); font-size: 11px; }.summary-value { color: var(--text-primary); font-family: 'Roboto Mono', monospace; }.sensor-card, .actuator-card { border-radius: 8px; background: var(--bg-secondary); border-color: var(--border-color); box-shadow: none; }.sensor-card:hover { border-color: rgba(34,211,238,.5); box-shadow: none; }.actuator-card:hover { border-color: rgba(245,158,11,.5); box-shadow: none; }.sensor-value { font-family: 'Roboto Mono', monospace; }.sensor-type-badge, .actuator-type-badge { border-radius: 4px; font-family: 'Roboto Mono', monospace; }.section-title { color: var(--text-primary); }.section-title::after { content: ' LIVE'; color: var(--text-muted); font: 10px 'Roboto Mono', monospace; letter-spacing: .1em; }.api-key-text { border: 1px solid var(--border-color); border-radius: 4px; background: var(--bg-secondary); }.history-item { border: 1px solid var(--border-color); border-radius: 6px; background: var(--bg-secondary); }.detail-state { border-radius: 8px; }
@media (max-width: 900px) { .summary-bar { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; }.summary-actions { margin-left: 0; }.sensor-cards :deep(.el-col), .actuator-cards :deep(.el-col) { width: 50%; max-width: 50%; flex: 0 0 50%; } }
@media (max-width: 560px) { .summary-bar { grid-template-columns: 1fr; }.sensor-cards :deep(.el-col), .actuator-cards :deep(.el-col) { width: 100%; max-width: 100%; flex: 0 0 100%; }.api-key-row { align-items: flex-start; flex-wrap: wrap; } }
</style>
