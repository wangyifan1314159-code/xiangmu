...
2. WebSocket 推送前端，设备列表状态即时变化`,
  ...
  },
  {
    ...
    title: 'TCP 接入',
    icon: Monitor,
    desc: 'Netty TCP 长连接通道，默认端口 1884。协议采用 UTF-8 JSON Lines：每条消息必须是一行完整 JSON，并以 LF（\\n）结束。',
    apis: [
      {
        method: 'CONN', path: 'tcp://{服务器IP}:1884',
        title: '协议约束与单设备认证',
        desc: '建立连接后 30 秒内必须发送 auth，否则服务端关闭连接。单行最大 65536 字节；认证后一个设备只保留一个 TCP 连接，新连接会替换旧连接。deviceId 与 apiKey 通过创建设备或重新生成 API Key 获取。',
...
$writer.WriteLine('{"type":"auth","deviceId":"dev_xxxxxxxx","apiKey":"<apiKey>"}')`,
...
{"type":"auth_result","success":false,"message":"设备ID或API Key不正确"}`,
      ...
      },
      {
        ...
        title: '上报传感器数据',
        desc: '仅在认证成功后发送。value 必须为有限数字，不能是字符串、NaN 或 Infinity；服务端使用与其他接入通道相同的数据写入与告警处理管线。',
        auth: '需先完成 auth',
        bodyParams: [
        ...
        example: `# 温度传感器（认证成功后发送）
{"type":"telemetry","sensorId":"temp_01","value":26.5,"sensorType":"temperature","unit":"C"}

# 甲烷传感器
{"type":"telemetry","sensorId":"methane_01","value":1200,"sensorType":"methane","unit":"ppm"}`,
        response: `遥测帧没有逐条成功回执。非法帧会收到：
{"type":"error","message":"遥测帧缺少 sensorId 或 value"}

数据可通过实时监控和历史数据接口查询；满足规则时会创建告警记录。`,
        id: 'tcp-telemetry'
      },
      {
        method: 'SEND', path: 'status / command_result 帧（设备 → 平台）',
        title: '状态同步与指令执行回执',
        desc: 'status 仅允许 ONLINE、OFFLINE、WARNING（大小写不敏感，服务端保存为大写）。command_result 为可选回执，当前会记录服务端日志。',
        auth: '需先完成 auth',
        bodyParams: [
          { name: 'type', type: 'string', required: true, desc: '"status" 或 "command_result"' },
          { name: 'status', type: 'string', required: false, desc: '"ONLINE" / "OFFLINE" / "WARNING"（type=status 时必填）' },
...
有效 status 会更新设备状态；command_result 当前记录到平台日志。`,
        id: 'tcp-status'
      },
      {
        method: 'RECV', path: 'command 帧（平台 → 设备）',
        title: '接收下行控制指令',
        desc: '管理端通过 POST /api/data/{deviceId}/command 下发时，平台优先写入该设备的在线 TCP 通道；设备不在线或发送失败时由业务服务回退 MQTT。设备收到 command 后执行，并建议回发 command_result。',
        auth: '—',
        example: `# 平台 → 设备（JSON 行，UTF-8）
{"type":"command","deviceId":"dev_xxxxxxxx","command":"on","params":{"actuator":"风扇"},"timestamp":1730000000000}

# 设备 → 平台（执行结束后回发）
{"type":"command_result","command":"on","success":true,"message":"executed"}`,
        response: `设备应按 command 和 params 执行，不应假设 params 固定字段。
TCP 通道写入成功仅表示消息已写入连接，不代表执行器已完成动作；以 command_result 作为设备侧执行结果。`,
        id: 'tcp-command'
      },
      {
        method: 'GATEWAY', path: 'gateway_auth 帧（网关 → 平台，可选）',
        title: '多设备网关认证',
        desc: '仅在部署配置了 APP_TCP_ACCESS_TOKEN 时可用。一个网关连接可代理多个已存在设备，deviceIds 不可为空、不可重复，且数量不能超过 APP_TCP_MAX_DEVICES（默认 256）。后续 telemetry/status 帧必须带 deviceId，且只能是已认证列表中的设备。',
        auth: 'gatewayId + accessToken + deviceIds',
        bodyParams: [
          { name: 'type', type: 'string', required: true, desc: '固定 "gateway_auth"' },
          { name: 'gatewayId', type: 'string', required: true, desc: '网关唯一标识' },
          { name: 'accessToken', type: 'string', required: true, desc: '部署时配置的 TCP 网关 Token' },
          { name: 'deviceIds', type: 'string[]', required: true, desc: '网关代理的已存在设备 ID 列表' }
        ] as ParamDef[],
        example: `{"type":"gateway_auth","gatewayId":"gw-line-01","accessToken":"<APP_TCP_ACCESS_TOKEN>","deviceIds":["dev_a","dev_b"]}

# 网关模式后，上报必须指定已授权的 deviceId
{"type":"telemetry","deviceId":"dev_a","sensorId":"temp_01","value":26.5,"sensorType":"temperature","unit":"C"}`,
        response: `{
  ...
  "message": "网关认证成功"
      ...
      },
      {
        ...
        title: '查询通道状态、连接与强制断开',
        desc: 'GET /api/tcp/status 返回通道启用状态、在线设备数和在线连接数。GET /api/tcp/connections：普通用户仅见自己的设备，管理员可见全部。仅管理员可 DELETE 强制断开。',
        auth: 'Bearer Token（断开仅限 ADMIN）',
        example: `# 通道摘要
curl -H "Authorization: Bearer {token}" "${BASE_URL}/api/tcp/status"

# 在线连接实例
curl -H "Authorization: Bearer {token}" "${BASE_URL}/api/tcp/connections"

# 强制断开（管理员）
curl -X DELETE -H "Authorization: Bearer {token}" \\
  "${BASE_URL}/api/tcp/connections/dev_xxxxxxxx"`,
        response: `{
  "success": true,
  "data": {
    "enabled": true,
...
连接列表字段包括 gatewayId、deviceId、deviceIds、connectionMode、remoteAddress、connectedAt、onlineSeconds。`,
