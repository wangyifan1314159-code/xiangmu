...
function Build-BackendProject {
    ...
    try {
        cmd.exe /c "mvn clean package -DskipTests 2>&1"
        if ($LASTEXITCODE -ne 0) {
...
function Deploy-StandardStack {
    ...
    Push-Location $ScriptDir
    try {
        cmd.exe /c "docker compose up -d --build 2>&1"
        if ($LASTEXITCODE -ne 0) {
...
function Deploy-BigDataStack {
    ...
    Push-Location $ScriptDir
    try {
        cmd.exe /c "docker compose -f docker-compose-bigdata.yml up -d 2>&1"
...
switch ($Mode) {
    ...
    "clean" {
        ...
        cmd.exe /c "docker compose down -v 2>&1"
        cmd.exe /c "mvn clean 2>&1"
        Write-Success "清理完毕"
    }
    default {
